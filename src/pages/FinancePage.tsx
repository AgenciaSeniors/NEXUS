...
export function FinancePage() {
  ...
  const COLORS = ['#1C1917', '#EA580C', '#F59E0B', '#EF4444', '#78716C'];

  if (activeShift === undefined || isInitialLoad) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#FAFAF9]">
        <div className="text-center">
          <Loader2 className="w-12 h-12 animate-spin text-[#1C1917] mx-auto mb-4" />
          <p className="text-[#78716C] font-semibold">Cargando datos del turno...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#FAFAF9]">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-gray-100 text-center animate-in fade-in zoom-in duration-300">
          <div className="w-16 h-16 bg-[#1C1917]/10 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner">
            <Lock className="text-[#1C1917] w-8 h-8" />
          </div>
          <h1 className="text-2xl font-black text-[#1C1917] mb-2">Apertura de Caja</h1>
          <p className="text-[#78716C] mb-6 text-sm">Inicia el turno para habilitar el punto de venta.</p>
          
          <div className="w-full text-left">
            <div className="mb-6 text-left">
              <label className="block text-xs font-bold text-[#78716C] uppercase mb-2 ml-1">Monto Inicial (Efectivo)</label>
              <div className="relative group">
                <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 text-[#78716C] w-5 h-5 group-focus-within:text-[#1C1917] transition-colors"/>
                <input 
                    type="number" step="0.01" autoFocus
                    className="w-full pl-10 pr-4 py-3 text-lg font-bold text-[#1C1917] border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#1C1917] outline-none transition-all" 
                    placeholder="0.00" 
                    value={amount} 
            ...
            <button 
                ...
                className="w-full bg-[#EA580C] hover:bg-[#EA580C]/90 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-[#EA580C]/20 flex justify-center items-center gap-2 active:scale-[0.98]"
            >
  ...
  return (
    <div className="p-4 md:p-6 pb-24 md:pb-6 min-h-screen bg-[#FAFAF9] print:bg-white print:p-0">
      
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 print:hidden">
        <div>
          <h1 className="text-2xl font-bold text-[#1C1917] flex items-center gap-2">
            <Unlock className="text-[#EA580C]" /> Caja Abierta
          </h1>
          <p className="text-[#78716C] text-xs font-mono mt-1 bg-white px-2 py-1 rounded inline-block border border-gray-200 shadow-sm">
        ...
        <div className="flex bg-white p-1.5 rounded-xl border border-gray-200 shadow-sm overflow-x-auto max-w-full scrollbar-hide">
          <button onClick={() => setViewMode('control')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${viewMode === 'control' ? 'bg-[#1C1917] text-white shadow-md' : 'text-[#78716C] hover:bg-gray-50'}`}>
            <Wallet size={16} /> Control
          </button>
          {isAdmin && (
            <>
              <div className="w-px h-6 bg-gray-200 mx-1 self-center"></div>
              <button onClick={() => setViewMode('history')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${viewMode === 'history' ? 'bg-[#1C1917] text-white shadow-md' : 'text-[#78716C] hover:bg-gray-50'}`}>
                <History size={16} /> Historial
              </button>
              <div className="w-px h-6 bg-gray-200 mx-1 self-center"></div>
              <button onClick={() => setViewMode('daily')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${viewMode === 'daily' ? 'bg-[#1C1917] text-white shadow-md' : 'text-[#78716C] hover:bg-gray-50'}`}>
                <Calendar size={16} /> Reportes
              </button>
              <button onClick={() => setViewMode('trends')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${viewMode === 'trends' ? 'bg-[#1C1917] text-white shadow-md' : 'text-[#78716C] hover:bg-gray-50'}`}>
                <BarChart3 size={16} /> Tendencias
              </button>
            </>
          )}
          <div className="w-px h-6 bg-gray-200 mx-1 self-center"></div>
          <button onClick={() => setViewMode('closing')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${viewMode === 'closing' ? 'bg-[#1C1917] text-white shadow-md' : 'text-[#78716C] hover:bg-gray-50'}`}>
        ...
        <div className="animate-in slide-in-from-bottom-4 duration-300 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
               <p className="text-[#78716C] text-[10px] font-bold uppercase tracking-wider mb-1 flex items-center gap-1"><ShoppingBag size={12}/> Total Ventas</p>
               <h3 className="text-2xl font-black text-[#1C1917]">{formatMoney(shiftStats.totalSales)}</h3>
               <p className="text-[10px] text-[#78716C] mt-1">Todas las confirmadas</p>
            </div>
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
               <p className="text-[#78716C] text-[10px] font-bold uppercase tracking-wider mb-1">Base Inicial</p>
               <h3 className="text-2xl font-black text-[#1C1917]">{formatMoney(shiftStats.startAmount)}</h3>
            </div>
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
               <p className="text-[#EA580C] text-[10px] font-bold uppercase tracking-wider mb-1">Ventas Efectivo</p>
               <h3 className="text-2xl font-black text-[#EA580C]">+{formatMoney(shiftStats.cashSales)}</h3>
            </div>
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
            ...
            <div className="bg-[#1C1917] p-5 rounded-2xl shadow-lg shadow-[#1C1917]/30 text-white relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
             <button onClick={() => { setMovementType('in'); setAmount(''); }} className="group flex items-center justify-center gap-3 p-4 bg-[#EA580C]/10 text-[#EA580C] rounded-2xl border border-[#EA580C]/20 hover:bg-[#EA580C]/20 font-bold transition-all active:scale-[0.98]">
                <div className="bg-[#EA580C]/20 text-[#EA580C] p-2 rounded-lg group-hover:bg-[#EA580C]/30 transition-colors"><PlusCircle size={20}/></div>
             ...
             <button onClick={() => setPinModal({isOpen: true, action: 'close'})} className="group flex items-center justify-center gap-3 p-4 bg-white text-[#1C1917] rounded-2xl border border-gray-200 hover:bg-gray-50 hover:border-gray-300 font-bold transition-all active:scale-[0.98]">
                <div className="bg-gray-100 text-[#78716C] p-2 rounded-lg group-hover:bg-gray-200 transition-colors"><Lock size={20} /></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <h3 className="font-bold text-[#1C1917] text-sm flex items-center gap-2"><ArrowRightLeft className="text-[#78716C]" size={16}/> Movimientos de Caja</h3>
                </div>
                <div className="overflow-x-auto max-h-60">
                    <table className="mobile-card-table w-full text-sm text-left">
                        <thead className="text-[#78716C] font-bold uppercase text-[10px] bg-[#FAFAF9] sticky top-0">
                        ...
                        {shiftData?.movements.map(m => (
                            <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                                <td className="p-3 text-[#78716C] text-xs font-mono" data-label="Hora">{new Date(m.created_at).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</td>
                                <td className="p-3" data-label="Tipo"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${m.type==='in'?'bg-[#EA580C]/10 text-[#EA580C] border-[#EA580C]/20':'bg-[#EF4444]/10 text-[#EF4444] border-[#EF4444]/20'}`}>{m.type === 'in' ? 'Entrada' : 'Salida'}</span></td>
                                <td className="p-3 font-medium text-[#1C1917] text-xs" data-label="Motivo">{m.reason}</td>
                                <td className={`p-3 text-right font-bold text-xs ${m.type==='in'?'text-[#EA580C]':'text-[#EF4444]'}`} data-label="Monto">{m.type==='out'?'-':'+'}{formatMoney(m.amount)}</td>
                            </tr>
                        ))}
                        {shiftData?.movements.length === 0 && <tr><td colSpan={4} className="p-8 text-center text-[#78716C] italic text-xs">Sin movimientos registrados</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <h3 className="font-bold text-[#1C1917] text-sm flex items-center gap-2"><ShoppingBag className="text-[#78716C]" size={16}/> Ventas Recientes</h3>
                    <span className="text-[10px] bg-[#1C1917]/10 text-[#1C1917] px-2 py-0.5 rounded-full font-bold">
                ...
                <div className="overflow-x-auto max-h-60">
                    <table className="mobile-card-table w-full text-sm text-left">
                        ...
                        {shiftData?.sales.filter(s => s.status !== 'voided').slice().reverse().map(s => (
                            <tr key={s.id} onClick={() => setSelectedTicket(s)} className="cursor-pointer hover:bg-[#1C1917]/5 transition-colors group">
                                <td className="p-3 text-[#78716C] text-xs font-mono group-hover:text-[#1C1917]" data-label="Hora">{new Date(s.date).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</td>
                                <td className="p-3" data-label="Método"><span className="px-2 py-0.5 bg-gray-100 text-[#78716C] rounded text-[10px] font-bold uppercase border border-gray-200">{s.payment_method}</span></td>
                                <td className="p-3 text-right font-bold text-[#1C1917] text-xs" data-label="Total">{formatMoney(s.total)}</td>
                            </tr>
                        ))}
                        {shiftData?.sales.filter(s => s.status !== 'voided').length === 0 && <tr><td colSpan={3} className="p-8 text-center text-[#78716C] italic text-xs">Sin ventas en este turno</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
          ...
          {shiftStats.hourlyChart.length > 1 && shiftStats.totalSales > 0 && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
              <h3 className="font-bold text-[#1C1917] text-sm flex items-center gap-2 mb-4">
                <BarChart3 size={16} className="text-[#1C1917]"/> Actividad del Turno por Hora
              </h3>
              <div className="h-40">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={shiftStats.hourlyChart} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                    ...
                    />
                    <Area type="monotone" dataKey="v" stroke="#1C1917" strokeWidth={2} fill="url(#shiftGrad)" dot={false} activeDot={{ r: 4, fill: '#1C1917' }}/>
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          {shiftStats.staffList.length > 1 && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
              <h3 className="font-bold text-[#1C1917] text-sm flex items-center gap-2 mb-4">
                <Trophy size={16} className="text-[#EA580C]"/> Ventas por Vendedor (Turno Actual)
              </h3>
                ...
                {shiftStats.staffList.map((s, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                      <div className="w-7 h-7 rounded-lg bg-[#1C1917]/10 text-[#1C1917] flex items-center justify-center text-[10px] font-black flex-shrink-0">
                        {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : s.name.substring(0, 2).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-1">
                          <p className="text-xs font-bold text-[#1C1917] truncate">{s.name}</p>
                          <span className="font-black text-xs text-[#EA580C] flex-shrink-0 ml-2">{formatMoney(s.total)}</span>
                        </div>
                        <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                          <div className="h-full rounded-full bg-[#EA580C] transition-all" style={{ width: `${pct}%` }}/>
                        </div>
                        <p className="text-[9px] text-[#78716C] mt-0.5">{s.count} venta{s.count !== 1 ? 's' : ''} · {pct.toFixed(0)}%</p>
                      </div>
                    </div>
                  );
                })}
           ...
           <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-5 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                  <h2 className="text-lg font-bold text-[#1C1917] flex items-center gap-2">
                      <History className="text-[#1C1917]"/> Historial (Últimos 30 días)
                  </h2>
                  <button
                    onClick={handleExportCSV}
                    className="flex items-center gap-2 px-3 py-2 bg-[#EA580C]/10 text-[#EA580C] border border-[#EA580C]/20 rounded-xl text-xs font-bold hover:bg-[#EA580C]/20 transition-colors"
              ...
              <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                      <thead className="text-[#78716C] font-bold uppercase text-xs bg-white border-b border-gray-200">
                          <tr>
                      ...
                      <tbody className="divide-y divide-gray-100">
                          {allSales.map(sale => (
                              <tr key={sale.id} className={`transition-colors ${sale.status === 'voided' ? 'bg-red-50/50 opacity-60' : sale.status === 'stock_conflict' ? 'bg-orange-50/60' : 'hover:bg-gray-50'}`}>
                                  <td className="p-4 font-mono text-[#78716C]">
                                  ...
                                  <td className="p-4 uppercase text-xs font-bold text-[#1C1917]">{sale.payment_method}</td>
                                  ...
                                  <td className="p-4 text-right font-black text-[#1C1917]">{formatMoney(sale.total)}</td>
                                  <td className="p-4">
                                      <div className="flex justify-center gap-2">
                                          <button onClick={() => setSelectedTicket(sale)} className="px-3 py-1.5 bg-gray-100 text-[#1C1917] rounded-lg font-bold text-xs hover:bg-gray-200 transition-colors">Ver</button>
                                          {sale.status === 'stock_conflict' && (
                                              <button
                                                onClick={async () => {
                                                  try {
                          ...
                          {allSales.length === 0 && <tr><td colSpan={5} className="p-8 text-center text-[#78716C]">No hay ventas registradas.</td></tr>}
                      </tbody>
                  </table>
              </div>
           </div>
        ...
        <div className="animate-in fade-in zoom-in-95 duration-300">
          <div className="flex justify-between items-center mb-6">
             <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-gray-200 shadow-sm">
                <button onClick={() => changeDate(-1)} className="p-2 hover:bg-gray-100 rounded-lg text-[#78716C]"><ArrowLeft size={20} /></button>
                <div className="relative mx-2">
                  <input type="date" value={selectedDate} max={today} onChange={(e) => e.target.value && setSelectedDate(e.target.value)} className="bg-transparent text-[#1C1917] font-bold outline-none cursor-pointer text-sm uppercase" />
                </div>
                <button onClick={() => changeDate(1)} disabled={selectedDate >= today} className="p-2 hover:bg-gray-100 rounded-lg text-[#78716C] disabled:opacity-30"><ArrowRight size={20} /></button>
             </div>
             <button onClick={() => syncPull()} className="p-2 bg-white text-[#1C1917] border border-gray-200 rounded-lg shadow-sm hover:bg-[#1C1917]/10 transition-colors" title="Sincronizar Nube"><RefreshCw size={20}/></button>
          </div>

          {/* KPIs */}
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
              <p className="text-[#78716C] text-[10px] font-bold uppercase mb-1 flex items-center gap-1"><Hash size={11}/> Tickets</p>
              <h3 className="text-2xl font-black text-[#1C1917]">{dailyStats.sales.length}</h3>
              <p className="text-[10px] text-[#78716C]">ventas del día</p>
            </div>
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
              <p className="text-[#78716C] text-[10px] font-bold uppercase mb-1 flex items-center gap-1"><DollarSign size={11}/> Ingresos</p>
              <h3 className="text-2xl font-black text-[#1C1917]">{formatMoney(dailyStats.revenue)}</h3>
              <p className="text-[10px] text-[#78716C]">neto del día</p>
            </div>
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
              <p className="text-[#78716C] text-[10px] font-bold uppercase mb-1 flex items-center gap-1"><TrendingDown size={11}/> Costos</p>
              <h3 className="text-2xl font-black text-[#1C1917]">{formatMoney(dailyStats.cost)}</h3>
              <p className="text-[10px] text-[#78716C]">costo de lo vendido</p>
            </div>
            <div className="col-span-2 lg:col-span-2 bg-gradient-to-br from-[#EA580C] to-[#C2410C] p-4 rounded-2xl shadow-lg shadow-[#EA580C]/20 text-white">
          ...
          {dailyStats.sales.length > 0 && (
            <div className="grid grid-cols-3 gap-3 mb-6">
              {[
                { label: 'Efectivo', value: dailyStats.paymentBreakdown.efectivo, color: 'text-[#EA580C]', bg: 'bg-[#EA580C]/10 border-[#EA580C]/20' },
                { label: 'Transferencia', value: dailyStats.paymentBreakdown.transferencia, color: 'text-[#F59E0B]', bg: 'bg-[#F59E0B]/10 border-[#F59E0B]/20' },
                { label: 'Tarjeta', value: dailyStats.paymentBreakdown.tarjeta, color: 'text-[#1C1917]', bg: 'bg-[#1C1917]/10 border-[#1C1917]/20' },
              ].map(item => (
                <div key={item.label} className={`p-3 rounded-xl border ${item.bg}`}>
                  <p className="text-[10px] font-bold text-[#78716C] uppercase mb-1">{item.label}</p>
                  <p className={`text-lg font-black ${item.color}`}>{formatMoney(item.value)}</p>
                  {dailyStats.revenue > 0 && <p className="text-[10px] text-[#78716C]">{((item.value / dailyStats.revenue) * 100).toFixed(0)}% del total</p>}
                </div>
              ))}
            </div>
          )}
          ...
          {dailyStats.sales.length > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              <div className="lg:col-span-2 bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                <h4 className="font-bold text-[#1C1917] text-sm mb-4 flex items-center gap-2"><BarChart3 size={16}/> Ventas por Hora</h4>
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                <h4 className="font-bold text-[#1C1917] text-sm mb-4 flex items-center gap-2"><Trophy size={16} className="text-[#F59E0B]"/> Top 5 Productos</h4>
                {dailyStats.topProducts.length === 0 ? (
                  <p className="text-xs text-[#78716C] text-center py-8">Sin datos</p>
                ) : (
                  <div className="space-y-3">
                    {dailyStats.topProducts.map((p, i) => {
                      ...
                      return (
                        <div key={i}>
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-bold text-[#1C1917] truncate flex-1 mr-2">
                              {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i+1}.`} {p.name}
                            </span>
                            <span className="text-[10px] font-black text-[#78716C] flex-shrink-0">{p.qty} un.</span>
                          </div>
                          <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div className="h-full rounded-full bg-[#1C1917] transition-all" style={{ width: `${pct}%` }}/>
                          </div>
                        </div>
                      );
                    })}
          ...
          {dailyStats.pieData.length > 0 && (
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 h-72 mb-6">
              <h4 className="font-bold text-[#1C1917] text-sm mb-4 flex items-center gap-2"><PieChartIcon size={16}/> Ingresos por Categoría</h4>
      ...
      {viewMode === 'trends' && (
        <div className="animate-in fade-in zoom-in-95 duration-300 space-y-6">
          ...
          <div className="flex gap-2">
            <button onClick={() => setTrendFilter('week')} className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${trendFilter === 'week' ? 'bg-[#1C1917] text-white shadow-md' : 'bg-white border border-gray-200 text-[#78716C] hover:bg-gray-50'}`}>Últimos 7 días</button>
            <button onClick={() => setTrendFilter('month')} className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${trendFilter === 'month' ? 'bg-[#1C1917] text-white shadow-md' : 'bg-white border border-gray-200 text-[#78716C] hover:bg-gray-50'}`}>Últimos 30 días</button>
          </div>

          {/* KPIs */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
              <p className="text-[#78716C] text-[10px] font-bold uppercase mb-1 flex items-center gap-1"><Hash size={11}/> Ventas</p>
              <h3 className="text-3xl font-black text-[#1C1917]">{trendStats.count}</h3>
              <p className="text-[10px] text-[#78716C]">tickets procesados</p>
            </div>
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
              <p className="text-[#78716C] text-[10px] font-bold uppercase mb-1 flex items-center gap-1"><DollarSign size={11}/> Ingresos</p>
              <h3 className="text-2xl font-black text-[#1C1917]">{formatMoney(trendStats.totalRevenue)}</h3>
              <p className="text-[10px] text-[#78716C]">total del período</p>
            </div>
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
              <p className="text-[#78716C] text-[10px] font-bold uppercase mb-1 flex items-center gap-1"><TrendingDown size={11}/> Costos</p>
              <h3 className="text-2xl font-black text-[#1C1917]">{formatMoney(trendStats.totalCost)}</h3>
              <p className="text-[10px] text-[#78716C]">costo del período</p>
            </div>
            <div className="bg-gradient-to-br from-[#EA580C] to-[#C2410C] p-5 rounded-2xl shadow-lg shadow-[#EA580C]/20 text-white">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <h4 className="font-bold text-[#1C1917] mb-1 text-sm">Evolución de Ingresos Netos</h4>
            <p className="text-[10px] text-[#78716C] mb-4">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendStats.chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                  ...
                  />
                  <Area type="monotone" dataKey="total" stroke="#EA580C" strokeWidth={2.5} fill="url(#trendGrad)" dot={{ r: 3, fill: '#EA580C', strokeWidth: 0 }} activeDot={{ r: 5, fill: '#EA580C' }}/>
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          {trendStats.topProducts.length > 0 && (
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <h4 className="font-bold text-[#1C1917] text-sm mb-5 flex items-center gap-2">
                ...
                {trendStats.topProducts.map((p, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-4">
                      <div className="w-7 flex-shrink-0 text-center text-sm font-black text-[#78716C]">
                        {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : <span className="text-xs">{i+1}</span>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-1.5">
                          <span className="text-sm font-bold text-[#1C1917] truncate">{p.name}</span>
                          <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                            <span className="text-[10px] text-[#78716C]">{p.qty} un.</span>
                            <span className="text-xs font-black text-[#EA580C]">{formatMoney(p.revenue)}</span>
                          </div>
                        </div>
                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full transition-all ${i === 0 ? 'bg-[#1C1917]' : i === 1 ? 'bg-[#EA580C]' : i === 2 ? 'bg-[#F59E0B]' : 'bg-gray-400'}`} style={{ width: `${pct}%` }}/>
                        </div>
                      </div>
                    </div>
                  );
      ...
      {viewMode === 'closing' && (
        <div className="animate-in slide-in-from-bottom-4 duration-300 max-w-3xl mx-auto">
          <div className="flex justify-between items-center mb-6 print:hidden">
            <h2 className="text-xl font-bold text-[#1C1917]">Cierre del Día</h2>
            <div className="flex items-center gap-3">
               <input type="date" value={selectedDate} max={today} onChange={(e) => e.target.value && setSelectedDate(e.target.value)} className="bg-white border border-gray-200 text-[#1C1917] font-bold rounded-lg p-2 outline-none text-sm" />
               <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 bg-[#1C1917] text-white text-xs font-bold rounded-lg hover:bg-[#1C1917]/90 transition-colors shadow-lg shadow-[#1C1917]/30"><Printer size={16} /> Imprimir</button>
            </div>
          </div>
          <div id="printable-report" className="bg-white p-8 rounded-none md:rounded-3xl shadow-lg border border-gray-200 print:shadow-none print:border-none print:p-0">
             <div className="text-center mb-8 border-b border-dashed border-gray-300 pb-6">
                 <h1 className="text-2xl font-black text-[#1C1917] uppercase tracking-widest mb-2">REPORTE Z</h1>
                 <p className="text-[#78716C] font-mono text-xs">Nexus POS</p>
                 <p className="text-sm text-[#78716C] mt-1">Fecha: {new Date(selectedDate).toLocaleDateString()}</p>
             </div>
             <div className="mb-8">
                 <div className="space-y-3 font-mono text-sm text-[#1C1917]">
                     ...
                     <div className="border-t-2 border-[#1C1917] pt-3 flex justify-between text-lg font-black mt-2"><span>TOTAL NETO</span><span>{formatMoney(closingStats.cashTotal + closingStats.transferTotal + closingStats.cardTotal)}</span></div>
                 </div>
             </div>
             <div>
                 <h3 className="text-xs font-bold text-[#78716C] uppercase mb-4 tracking-wider">Desglose de Productos Vendidos</h3>
                 {closingStats.productsList.length > 0 ? (
                     <table className="w-full text-sm font-mono text-[#1C1917]">
                         <thead className="text-[#78716C] border-b border-gray-200 text-[10px] uppercase">
                 ...
                 ) : <p className="text-center text-[#78716C] italic text-xs">Sin movimientos.</p>}
             </div>

             {closingStats.staffList.length > 1 && (
               <div className="mt-6 pt-5 border-t border-dashed border-gray-200">
                 <h3 className="text-xs font-bold text-[#78716C] uppercase mb-3 tracking-wider">Ventas por Vendedor</h3>
                 <div className="space-y-2">
                   {closingStats.staffList.map((s, i) => (
                     <div key={i} className="flex items-center justify-between p-2.5 bg-gray-50 rounded-xl border border-gray-100">
                       <div className="flex items-center gap-2">
                         <div className="w-7 h-7 rounded-lg bg-[#1C1917]/10 text-[#1C1917] flex items-center justify-center text-[10px] font-black">
                           {s.name.substring(0, 2).toUpperCase()}
                         </div>
                         <div>
                           <p className="text-xs font-bold text-[#1C1917]">{s.name}</p>
                           <p className="text-[10px] text-[#78716C]">{s.count} venta{s.count !== 1 ? 's' : ''}</p>
                         </div>
                       </div>
                       <span className="font-black text-sm text-[#1C1917]">{formatMoney(s.total)}</span>
                     </div>
                   ))}
                 </div>
               </div>
      ...
      {pinModal.isOpen && (
        <div className="fixed inset-0 bg-[#1C1917]/80 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-white rounded-3xl p-6 max-w-xs w-full shadow-2xl scale-100 animate-in zoom-in-95 duration-200">
              <div className="text-center mb-6">
                  ...
                  <h2 className="text-xl font-black text-[#1C1917]">Acceso Restringido</h2>
                  <p className="text-xs text-[#78716C] mt-1">Ingresa el PIN Maestro para continuar</p>
              </div>
              
              <div className="mb-6">
                  <div className="flex justify-center gap-3 mb-2">
                      {[0,1,2,3].map(i => (
                          <div key={i} className={`w-4 h-4 rounded-full transition-all ${pinInput.length > i ? 'bg-[#1C1917] scale-110' : 'bg-gray-200'}`}></div>
                      ))}
                  </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                  {[1,2,3,4,5,6,7,8,9].map(num => (
                      <button key={num} disabled={pinLockSecondsLeft > 0} onClick={() => { if(pinInput.length < 4) setPinInput(pinInput + num) }} className="py-4 bg-gray-50 hover:bg-gray-100 rounded-xl text-xl font-black text-[#1C1917] transition-colors active:scale-95 disabled:opacity-30">
                          {num}
                      </button>
                  ))}
                  ...
                  <button disabled={pinLockSecondsLeft > 0} onClick={() => { if(pinInput.length < 4) setPinInput(pinInput + '0') }} className="py-4 bg-gray-50 hover:bg-gray-100 rounded-xl text-xl font-black text-[#1C1917] transition-colors active:scale-95 disabled:opacity-30">
                      0
                  </button>
                  <button disabled={pinLockSecondsLeft > 0} onClick={() => setPinInput(pinInput.slice(0, -1))} className="py-4 bg-gray-50 hover:bg-gray-100 rounded-xl text-[#78716C] font-bold transition-colors flex items-center justify-center disabled:opacity-30">
                      <ArrowLeft size={24}/>
                  </button>
              </div>
              ...
              <button
                  ...
                  className="w-full py-4 bg-[#1C1917] text-white font-bold rounded-xl shadow-lg transition-all active:scale-[0.98] disabled:opacity-50"
              >
      ...
      {voidConfirmSale && (
        <div className="fixed inset-0 bg-[#1C1917]/70 z-[90] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-lg font-black text-[#1C1917]">¿Anular esta venta?</h2>
                <p className="text-xs text-[#78716C]">Esta acción requiere PIN Maestro</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-xl p-3 mb-5 text-xs space-y-1 border border-gray-100">
              <div className="flex justify-between"><span className="text-[#78716C]">Ticket</span><span className="font-bold font-mono">#{voidConfirmSale.id.slice(0,8).toUpperCase()}</span></div>
              <div className="flex justify-between"><span className="text-[#78716C]">Total</span><span className="font-black text-[#1C1917]">{formatMoney(safeFloat(voidConfirmSale.total))}</span></div>
              <div className="flex justify-between"><span className="text-[#78716C]">Método</span><span className="font-bold uppercase">{voidConfirmSale.payment_method}</span></div>
              <div className="flex justify-between"><span className="text-[#78716C]">Items</span><span className="font-bold">{voidConfirmSale.items?.length || 0} producto(s)</span></div>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 bg-gray-100 text-[#1C1917] rounded-xl font-bold hover:bg-gray-200 transition-colors"
              >
                Cancelar
              </button>
              <button
      ...
      {/* MODAL MOVIMIENTOS */}
      {movementType && (
        <div className="fixed inset-0 bg-[#1C1917]/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl">
                <div className="flex justify-between items-center mb-4">
                    <h2 className={`text-xl font-black flex items-center gap-2 ${movementType === 'in' ? 'text-[#EA580C]' : 'text-[#EF4444]'}`}>
                ...
                <div className="space-y-4">
                    <div>
                        <label className="block text-[10px] font-bold text-[#78716C] uppercase mb-1">Monto</label>
                    ...
                    <div>
                        <label className="block text-[10px] font-bold text-[#78716C] uppercase mb-1">Motivo</label>
                    ...
                    <button type="button" onClick={handleMovement} disabled={isLoading || amount === '' || reason === ''} className="w-full py-3.5 font-bold text-white rounded-xl bg-[#1C1917]">
                        CONFIRMAR
                    </button>
                </div>
            </div>
      ...
      {isClosing && (closingShiftStats || shiftStats) && (() => {
        ...
        return (
        <div className="fixed inset-0 bg-[#1C1917]/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl p-0 max-w-md w-full shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
                <div className="bg-[#1C1917] p-6 text-white flex justify-between items-center sticky top-0 z-10">
                    <h2 className="text-xl font-black flex items-center gap-2"><Lock className="text-[#EA580C]"/> CORTE DE CAJA</h2>
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div>
                      <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-3">
                          <div className="flex justify-between items-center">
                              ...
                              <span className="text-lg font-black text-[#1C1917]">{formatMoney(ss.expectedCash)}</span>
                          </div>
                      ...
                      <label className="block text-xs font-bold text-[#78716C] uppercase mb-2">Conteo real de efectivo</label>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#78716C] text-xl font-bold">$</span>
                        <input type="number" step="0.01" autoFocus value={amount} onChange={e => setAmount(e.target.value)} className="w-full pl-10 pr-4 py-3.5 text-2xl font-black border-2 rounded-xl focus:border-[#1C1917] outline-none transition-colors" placeholder="0.00" />
                      </div>
                    ...
                    <div>
                      ...
                      <label className="block text-xs font-bold text-[#78716C] uppercase mb-2">Monto verificado en transferencias</label>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#78716C] text-xl font-bold">$</span>
                    ...
                    {amount !== '' && transferCount !== '' && (
                      <div className={`p-4 rounded-xl border-2 ${(cashDiffPreview! + transferDiffPreview!) === 0 ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200'}`}>
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-black text-[#1C1917] uppercase">Diferencia Total</span>
                    ...
                    <button type="button" onClick={handleCloseShift} disabled={isLoading || amount === '' || transferCount === ''} className="w-full py-4 bg-[#1C1917] text-white font-black rounded-xl text-lg hover:bg-[#0C0A09] transition-all active:scale-95 disabled:opacity-50 shadow-lg shadow-[#1C1917]/20">
