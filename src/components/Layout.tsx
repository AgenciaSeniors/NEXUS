...
export function Layout({ currentStaff, onChangeStaff }: LayoutProps) {
  ...
  const getButtonState = () => {
    ...
    return { className: "bg-[#16A34A]/10 text-[#16A34A] ring-1 ring-[#16A34A]/30 hover:bg-[#16A34A]/20", icon: <CheckCircle2 size={20} />, title: "Sistema actualizado y seguro" };
  };

  const buttonState = getButtonState();

  return (
    <div className="flex h-screen w-full bg-[#FAFAF9] overflow-hidden font-sans text-[#1C1917]">

      {/* SIDEBAR DESKTOP */}
      <aside className={`hidden md:flex flex-col items-center py-6 z-20 shadow-xl transition-all bg-[#1C1917] text-white duration-300 ${isCashier ? 'w-20' : 'w-24'}`}>

        {/* LOGO PRINCIPAL */}
        <div className="mb-6 p-1.5 bg-white rounded-2xl shadow-lg flex items-center justify-center w-14 h-14 overflow-hidden border-2 border-[#EA580C]/30">
          <img
              src={logo}
              alt="Logo"
        ...
        <div className="mb-8 text-center px-1 w-full relative">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-1 font-bold text-sm shadow-lg transition-colors border-2 border-[#EA580C] ${isAdmin ? 'bg-[#EA580C] text-white' : 'bg-white/10 text-white'}`}>
            {currentStaff?.name.substring(0, 2).toUpperCase() || 'NX'}
          </div>
          {!isCashier && (
             <p className="text-[10px] font-bold text-[#EA580C] truncate w-full px-1 uppercase tracking-wider opacity-90">
        ...
        <nav className="flex-1 flex flex-col gap-4 w-full px-3">
          {menuItems.filter(i => i.show).map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} className={`flex flex-col items-center justify-center p-3 rounded-xl transition-all duration-200 group relative ${isActive ? 'bg-[#EA580C] text-white shadow-lg shadow-[#EA580C]/20 font-bold translate-x-1' : 'text-gray-300 hover:text-white hover:bg-white/10'}`}>
                {item.icon}
                {showInventoryBadge && (
                ...
                <span className="absolute left-full ml-4 px-3 py-2 bg-[#1C1917] text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-xl font-bold uppercase tracking-wide border border-gray-700">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        <header
          className="bg-[#1C1917] text-white border-b border-[#1C1917] px-4 pb-3 flex justify-between items-center md:hidden z-10 shadow-lg sticky top-0"
          style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 0.75rem)' }}
        >
            <button onClick={() => setIsMobileMenuOpen(true)} className="text-white hover:text-[#EA580C] transition-colors">
                <Menu size={26} />
            </button>
            ...
            <div className="flex items-center gap-3">
                <button onClick={handleManualSync} disabled={!isOnline || isSyncing} className={`flex items-center justify-center w-9 h-9 rounded-full transition-all ${!isOnline ? 'bg-red-500 text-white' : isSyncing || pendingCount > 0 ? 'bg-[#F59E0B] text-white animate-pulse' : 'bg-[#16A34A] text-white'}`}>
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden relative bg-[#FAFAF9] scroll-smooth pb-safe">
        ...
        >
            {menuItems.filter(i => i.show).slice(0, 4).map((item) => {
              ...
              const showInventoryBadge = item.path === '/inventario' && inventoryAlertCount > 0;
              return (
                <Link key={item.path} to={item.path} className={`flex-1 p-2 rounded-xl flex flex-col items-center transition-all duration-200 relative ${isActive ? 'text-[#EA580C] bg-[#EA580C]/5 transform -translate-y-1' : 'text-gray-400 hover:text-[#EA580C]'}`}>
        ...
        {isMobileMenuOpen && (
            <div className="fixed inset-0 z-50 md:hidden flex justify-end">
                <div className="absolute inset-0 bg-[#1C1917]/90 backdrop-blur-sm animate-in fade-in" onClick={() => setIsMobileMenuOpen(false)} />

                <div className="relative bg-[#FAFAF9] w-4/5 max-w-xs h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 border-l border-[#E7E5E4]">
                    <div
                      className="px-6 pb-6 bg-[#1C1917] text-white"
                    ...
                    >
                        ...
                        <p className="text-[#EA580C] text-sm font-medium">Hola, {currentStaff?.name.split(' ')[0]}</p>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        {menuItems.filter(i => i.show).map(item => {
                             const isActive = location.pathname === item.path;
                             return (
                                <Link key={item.path} to={item.path} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-4 p-4 rounded-xl transition-all ${isActive ? 'bg-[#1C1917] text-white shadow-md font-bold' : 'text-[#1C1917] hover:bg-[#F5F5F4]'}`}>
                                    <span className={isActive ? 'text-[#EA580C]' : 'text-[#EA580C]/70'}>{item.icon}</span>
                    ...
                    <div className="p-6 border-t border-[#E7E5E4] bg-white space-y-3">
                        {multipleStaff && onChangeStaff && (
                          <button
                            onClick={() => { setIsMobileMenuOpen(false); onChangeStaff(); }}
                            className="flex items-center justify-center gap-3 text-[#1C1917] w-full p-4 hover:bg-[#1C1917]/5 rounded-xl font-bold transition-colors border border-[#1C1917]/20"
                          >
      ...
      {showGuidePrompt && (
        <div className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-sm z-50 animate-in slide-in-from-bottom-4 fade-in duration-300">
          <div className="bg-[#1C1917] text-white rounded-2xl shadow-2xl p-4 flex items-center gap-3">
            ...
            <div className="flex gap-2 flex-shrink-0">
              <button
                ...
                className="bg-[#EA580C] text-white text-xs font-black px-3 py-2 rounded-xl hover:bg-[#EA580C]/90 transition-colors whitespace-nowrap"
              >
                Ver guía
              </button>
              <button
